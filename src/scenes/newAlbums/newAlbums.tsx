...
import { getNewReleases } from 'api/data/browse'
...
export const NewAlbums: FC = () => {
  const savedMediaProps = useGetMediaList<Album>(getNewReleases)
  const { totalCount, isLoading } = savedMediaProps
  const [renderedAlbumSnippetId, setRenderedAlbumSnippetId] =
    useState<string>('')

  const mediaTitle = strings.scenes.newAlbums.newReleases
